...
if (document.querySelector('.filters-form')) {
    document.querySelector('.filters-form').onsubmit = event => {
        event.preventDefault();
        let url = document.querySelector('.filters-form').action + '//' + document.querySelector('input[name="search"]').value + '/';
        ...
        url += '/' + (document.querySelector('#activation_pending').checked ? document.querySelector('#activation_pending').value : '') + '/';
        if (document.querySelector('#role') && document.querySelector('#role').checked) {
            url += document.querySelector('#role').value
        }
        url += '/{{ $order }}/{{ $order_by }}/1';
