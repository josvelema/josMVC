...
class View {
	...
	static function cache($file) {
		if (!file_exists(BASE_PATH . '/' . cache_path)) {
		  	mkdir(BASE_PATH . '/' . cache_path, 0744);
		}
	    $cacheLocation = BASE_PATH . '/' . cache_path . str_replace(array('/', '.html'), array('_', ''), $file . '.php');
	...
	static function clearCache() {
		foreach(glob(BASE_PATH . '/' . cache_path . '*') as $file) {
			unlink($file);
		}
	}
	...
	static function compileStatic($code) {
		return preg_replace('/{% ?static ?\'?(.*?)\'? ?%}/i', '<?php echo "' . App::root_url() . '/app/static/$1" ?>', $code);
	}
